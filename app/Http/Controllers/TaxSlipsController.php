...
class TaxSlipsController extends Controller
{
    /**
    ...
    {
        Audit::log(Auth::user()->id, request()->ip(), trans('slips/tax/general.audit-log.category'), trans('slips/tax/general.audit-log.msg-index'));

        $page_title = trans('slips/tax/general.page.index.title');
        $page_description = trans('slips/tax/general.page.index.description');
        ...
        return view('slips.tax.index', compact('agent_position_lists', 'month_lists', 'year_lists', 'page_title', 'page_description'));
    }

    public function export(\App\Http\Requests\TaxExcelExport $export)
    {
        Audit::log(Auth::user()->id, request()->ip(), trans('slips/tax/general.audit-log.category'), trans('slips/tax/general.audit-log.msg-export'));
        ...
        $year = Input::get('year');
        $isCalculated = TaxReport::isSlipCalculated($period, $month, $year);
        $recalc = false;
        ...
        if(\Input::has('recalc')) {
            if(!TaxReport::isRecalculationAllowed($period, $month, $year) && $isCalculated) {
        ...
        foreach($agents as $agent) {
            $commissions[$agent->id] = new Tax($agent, $period, $month, $year);
