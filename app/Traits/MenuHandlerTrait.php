...
trait MenuHandlerTrait
{
    ...
    public function getLeafMenuItem( $leaf = null )
    {
        ...
        $leaf = $this->menuRepository->findBy('name', \Request::route()->getName());
        if ($leaf instanceof Menu) {
            return $leaf;
        }

        // Could not find the requested menu item, throwing an exception.
        throw new MenuBuilderMenuItemNotFoundException("Menu item [".$leaf."] not found for route name [".\Request::route()->getName()."].");
    ...
    public function getVariables( Menu $menu, $variables = [] )
    {
        ...
        $variables = [  'URL'         => Config::get('app.url') . $url,
                        'ICON'        => $icon,
                        'LABEL'       => $menu->label,
                        'CURRENT_URL' => Config::get('app.url') . $currentUrl
        ];
