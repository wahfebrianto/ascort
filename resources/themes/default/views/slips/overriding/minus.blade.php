    ...
    <div class='row'>
        <div class='col-md-6'>
            <div class="box box-info">
                <div class="box-header with-border">
                    <h3 class="box-title">{{ trans('slips/overriding/general.page.minus.input-title') }}</h3>
        ...
        <div class='col-md-6'>
            <div class="box box-primary fa-bg fa-bg-export fa-bg-small">
                <div class="box-header with-border">
                    <h3 class="box-title">{{ trans('slips/overriding/general.page.minus.export-title') }}</h3>
                    <div class="box-tools pull-right">
                        <button class="btn btn-box-tool" data-widget="collapse" data-toggle="tooltip" title="Collapse"><i class="fa fa-minus"></i></button>
                    </div>
                </div>
                <div class="box-body" id="form-container">
                    ...
                    {!! Form::open( ['route' => 'slips.overriding.export', 'id' => 'form_export_sales', 'method' => 'GET', 'class' => 'form-horizontal'] ) !!}
                    {!! Form::hidden('agent_id', Input::get('agent_id')) !!}
                    {!! Form::hidden('agent_position_id', Input::get('agent_position_id')) !!}
                    {!! Form::hidden('period', Input::get('period')) !!}
                    {!! Form::hidden('month', Input::get('month')) !!}
                    ...
                    <table class="table table-stripped table-bordered">
                        ...
                        <tbody>
                        @if(!Session::has('ovr.minus') || count(Session::get('ovr.minus')) == 0)
                            <tr>
                                <td colspan="4" class="text-center warning">No minus data</td>
                            </tr>
                        @else
                            @foreach(Session::get('ovr.minus') as $key => $value)
                                <tr>
                                    <td>{{ $value['agent']->id }}</td>
                                    <td>{{ $value['agent']->name }}</td>
                                    <td>{{ \App\Money::format('%(.2n', $value['value']) }}</td>
                                    <td><a href="{{ route('slips.overriding.delete_minus', ['key' => $key]) }}" class="btn btn-sm btn-danger"><i class="fa fa-times"></i> Delete</a></td>
                                </tr>
                            @endforeach
                        @endif
                        </tbody>
