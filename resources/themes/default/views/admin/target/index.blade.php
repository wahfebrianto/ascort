        ...
        <div class='col-sm-12 col-md-12'>
            ...
            <!-- Box -->
                ...
                <div class="box-header with-border">
                    <h3 class="box-title">{{ trans('admin/target/general.page.index.table-title') }}</h3>
                    &nbsp;|&nbsp;
                    @include('partials._body_top_grid_general_action', ['module_name' => 'admin.target', 'create_title' => trans('admin/target/general.button.create'), 'enabledOnly' => $enabledOnly])
                        ...
                        (new GridConfig)
                            ...
                            ->setColumns([
                                ...
                                (new FieldConfig)
                                    ...
                                    ->setCallback(
                                            function($val, DataRow $row) {
                                                return \Html::link(route('admin.target.show', ['id' => $row->getCellValue('id')]), $val);
                                            }
                                    )
                                ,
                                (new FieldConfig)
                                    ...
                                    ->setCallback(
                                            function($val, DataRow $row) {
                                                return \Html::link(route('admin.target.show', ['id' => $row->getCellValue('id')]), $row->getCellValue('agent_position')['name']);
                                            }
                                    )
                                ,
                                (new FieldConfig)
                                    ->setName('target_amount')
                                    ...
                                    ->setCallback(
                                            function($val, DataRow $row) {
                                                return \Html::link(route('admin.target.show', ['id' => $row->getCellValue('id')]), \App\Money::format('%(.2n', $val));
                                            }
                                    )
                                ,
                                (new FieldConfig)
                                    ...
                                    ->setCallback(
                                        function($val, DataRow $row) {
                                            ...
                                            $showAction = html_entity_decode(
                                                        \Html::link(route('admin.target.show', $id), '<span class="fa fa-eye"></span> ' . trans('general.button.view'), [
                                                                'class' => 'btn btn-sm btn-default btn-action',
                                                        ])
                                                ) . ' ';
                                            $editAction = html_entity_decode(
                                                    \Html::link(route('admin.target.edit', $id), '<span class="fa fa-edit"></span> ' . trans('general.button.edit'), [
                                                        'class' => 'btn btn-sm bg-purple btn-action',
                                                    ])
                                                ) . ' ';
                                            if ($row->getCellValue('is_active') == 1) {
                                                $enableAction = html_entity_decode(
                                                    \Html::link(route('admin.target.disable', $id), '<span class="fa fa-trash"></span> ' . trans('general.button.delete'), [
                                                ...
                                                $enableAction = html_entity_decode(
                                                    \Html::link(route('admin.target.enable', $id), '<span class="fa fa-share"></span> ' . trans('general.button.restore'), [
...
@section('body_bottom')
    @include('partials._body_bottom_grid_general_checkbox', ['module_name' => 'admin.target'])
@endsection
